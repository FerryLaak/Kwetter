...
public class UserTest {
    ...
    public void testAddKweet() {
        System.out.println("Adding a kweet");
        Timestamp timestamp = new Timestamp(System.currentTimeMillis());
        user1.addKweet(kweet1);
        assertEquals("Kweets should not be null", kweet1, user1.getKweets().get(0));
    }

    @Test
    public void testRemoveKweet(){
        System.out.println("Removing a kweet");
        Timestamp timestamp = new Timestamp(System.currentTimeMillis());
        user2.addKweet(kweet2);
        assertEquals("Kweets should not be null", kweet2, user2.getKweets().get(0));

        user2.removeKweet(kweet2);
        assertEquals("Amount of tweets should be 0", 0, user2.getKweets().size());
    }

    @Test
    public void testFollowing(){
        System.out.println("getFollowing");
        assertEquals(2, user2.getFollowing().size());
    }

    @Test
    public void testRemoveFollowing(){
        System.out.println("remove Following");
        user2.removeUser(user3);
